...
def analyze_text(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Language: auto is stubbed for now, we will add real detection later.
    language = preset.language if preset.language_mode == "fixed" and preset.language else "auto"

    run_id = new_run_id()
    run_folder = make_run_folder(RUNS_BASE, run_id)

    redacted_text, findings, summary = analyze_layer1_text(text, preset, language)
    ...
    write_json(os.path.join(run_folder, "run_report.json"), build_run_report(run_id, preset, summary, file_count=1))
    write_findings_csv(os.path.join(run_folder, "findings.csv"), run_id, file_hash, findings)

    # Also write the redacted output as a text file for convenience
    out_path = os.path.join(run_folder, "output", "TEXT_0001_redacted.txt")
